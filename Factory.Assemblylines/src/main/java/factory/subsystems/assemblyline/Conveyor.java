...
public class Conveyor implements ConveyorInterface, ContainerDemander {
	...
	private static final int MAXIMUM_LUBRICANT_PER_START = 15;
	
	/** The wait time (in ms) before sending another {@link EventKind#LACK_OF_MATERIAL} event. */
	private static final int EVENT_SENDING_INTERVAL = 30_000;
	...
	public void start() {
		...
		if (lubricantAmount < (MAXIMUM_LUBRICANT_PER_START * 5)) {
			if (Arrays.asList(assemblyLine.getOutputBox().getStoredMaterials()).contains(Material.LUBRICANT)) {
				//take lubricant out of the box
				this.receiveContainer(assemblyLine.getOutputBox().deliverContainer(Material.LUBRICANT));
			} else {
				//send event that we need some lubricant
				if ((System.currentTimeMillis() - lastLackOfLubricantEventSent) > EVENT_SENDING_INTERVAL) {
